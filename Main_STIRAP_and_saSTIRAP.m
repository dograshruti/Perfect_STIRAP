...
if (set1==1 || abs(r)>=1)
nti = 1/r*log(epsilon/sqrt(1-epsilon^2)) + r/2 - 1/r*log(alpha); % calculated
ntf = 1/r*log(epsilon/sqrt(1-epsilon^2)) + r/2 + 1/r*log(alpha); % calculated
omega = -r*exp(r^2/8)/2/(2-sqrt(2))/sigma;     % calculated
elseif (set2==1 & abs(r)<1)
nti = -1*log(epsilon/sqrt(1-epsilon^2)) + r/2 - 1/r*log(alpha);
ntf = -1*log(epsilon/sqrt(1-epsilon^2)) + r/2 + 1/r*log(alpha);
omega = 1*exp(r^2/8)/2/(2-sqrt(2))/sigma;
end
...
sa=0 ;       % 1,0. 1- set Counterdiabatic drive Hamiltonian
dc02=0;      % 1,0. 1- direct coupling between levels 0-2
cosh02=0;    % 1,0. 1- omega_{02}(t) is approximated to cosh pulse
area_plot=0; % 1,0. 1- set STIRAP/saSTIRAP p_2 surface map vs A and A_{02}
